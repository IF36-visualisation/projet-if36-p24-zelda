...
-   Popularité est-elle liée à la forme de l'œuvre ? Est-ce que, pour des œuvres ayant la même forme, qu'il s'agisse d'un single ou d'un album, une fréquence plus élevée de publication par les artistes conduit à un plus grand nombre d'écoutes ?

##### Traiter le format des dates dans les données
```{r}
dataset_if36 <- dataset_if36 %>%
  filter(!is.na(as.Date(date, "%Y-%m-%d")))
dataset_if36$date <- as.Date(dataset_if36$date, "%Y-%m-%d")
...
##### Regroupez par artiste et par année pour calculer le nombre de sorties et le nombre total de streams par an
```{r}
...
##### Calculer la fréquence de sortie annuelle moyenne et le nombre total de streams moyenne pour chaque artiste
```{r}
artist_stats_filtered <- artist_yearly_filtered %>%
  group_by(Artist) %>%
  summarise(Avg_Frequency_2019_2023 = mean(Release_Frequency), Avg_Streams_2019_2023 = mean(Total_Streams))
```

##### Tracez un nuage de points de la fréquence moyenne des sorties d'albums et du nombre moyen de streams
```{r}
ggplot(artist_stats_filtered, aes(x = Avg_Frequency_2019_2023, y = Avg_Streams_2019_2023)) +
  geom_point(alpha = 0.5, size = 0.8) +
  labs(title = "Average Yearly Release Frequency vs. Average Yearly Streams (2019-2023)",
