...
stream_plot <- df_q1 %>%
  ...
  stat_summary(aes(group = 1), fun = mean, geom = 'line', colour = 'orange', alpha = 0.7) +
  stat_summary(aes(group = 1), fun = median, geom = 'line', colour = 'yellow', alpha = 0.7) +
  labs(x = "Année", y = "Nombre de Streams", title = "Streams par année de publication") +
  coord_trans(y = 'log10') +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 6))
...
view_plot <- df_q1 %>%
  ...
  stat_summary(aes(group = 1), fun = mean, geom = 'line', colour = 'orange', alpha = 0.7) +
  stat_summary(aes(group = 1), fun = median, geom = 'line', colour = 'yellow', alpha = 0.7) +
  labs(x = "Année", y = "Nombre de Vues", title = "Vues par année de publication") +
  coord_trans(y = 'log10') +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 6))

#on utilise le package patchwork pour afficher les deux plots ensembles
...
df_q1 %>%
  ...
  geom_violin(color = "darkcyan", fill = "lightblue") +
  stat_summary(aes(group = 1), fun = mean, geom = 'line', colour = 'orange') +
  stat_summary(aes(group = 1), fun = median, geom = 'line', colour = 'yellow') +
...
df_q1 %>%
    ...
    labs(x = "Année",
         y = "Count",
         title = "Distribution des années de publication dans le jeu de données")
```
